param (
    ...
    [string]$OutputFile = "NTFS_Permission_Audit_rev01e2.ndjson"
)

$SanityCheck = $false # $true enable $SanityCheck Function $false disable $SanityCheck Function - added rev01e
...
#SanityCheck Function - added rev01e -->
function Compare-AccessEntries {
    param (
        [Parameter(Mandatory=$true)] $ParentACEs,
        [Parameter(Mandatory=$true)] $ChildACEs,
        [Parameter(Mandatory=$true)] [string]$Path
    )

    foreach ($p in $ParentACEs) {
        foreach ($c in $ChildACEs) {
            if ($p.Identity -eq $c.Identity -and $p.AccessControlType -eq $c.AccessControlType) {
                Write-Host "`n🧪 Comparing ACEs:" -ForegroundColor DarkYellow
                Write-Host "   Path: $Path"
                Write-Host "   Identity: $($p.Identity)"
                Write-Host "   Rights: $($p.Rights) vs $($c.Rights)"
                Write-Host "   InheritanceFlags: $($p.InheritanceFlags) vs $($c.InheritanceFlags)"
                Write-Host "   PropagationFlags: $($p.PropagationFlags) vs $($c.PropagationFlags)"
            }
        }
    }
}
#End Sanity Check Function /-->

#Modular Explicit Issue Logging Start added rev01d -->
$IssueLogFile = Join-Path $OutputPath "NTFS_Permission_Issues_rev01e2.csv"
        ...
        if ($metadata.InheritanceEnabled -eq $true -and $metadata.ParentAcl.Count -gt 0) {
            ...
            foreach ($pAce in $parentACEs) {
                ...
                if (-not $match) {
                    ...
                    foreach ($m in $match) {
                        if ($SanityCheck) {
                            Compare-AccessEntries -ParentACEs @($pAce) -ChildACEs $match -Path $Path #Swapped out explicit Sanity Check with Modular, on demand Sanity Check fuction all - add rev01e
                            }
